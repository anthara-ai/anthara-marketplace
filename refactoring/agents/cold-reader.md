...
6. **What worked.** Two or three things, so the fix does not remove them.

Then the verdict on one line: **ready** when no slide has a stop rated above 3, no term is used on a slide before its gloss, and no sum on the page fails, otherwise **not ready** followed by the fixes that block readiness, one per line, each as "change X to Y" with X quoted from the page. Appendix findings below that bar are still reported, as advice rather than blockers.
